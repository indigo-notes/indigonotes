...
def sign_up(username: str, password: str, confirm_password: str, email_address: str):
    res = sign_up_user(username, password, confirm_password, email=email_address)
...
def upload_note_to_supa(note: str):
    ...
    if len(data)> 0 and len(data) == 100:
        ...
        supa.table("notes").insert({"note": encrypt_note(note), "user": req.username, "number": len(data)+1}).execute()
